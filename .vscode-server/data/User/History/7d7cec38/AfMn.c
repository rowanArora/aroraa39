...
/* Helper functions and macros only! */
/* Add code BEGIN */
// int find_distance(int write, int read) {
// 	if (write == read) {
// 		return 0;
// 	}
// 	else if (read < write && read >= 0 && write >= 0) {
// 		return write - read;
// 	}
// 	else {
// 		int counter = 0;
...
/* Add code BEGIN */
	...
	// else if (find_distance(writeIndex, readIndex) == PART_B_MAX_SIZE) {
	// 	return ERR_FULL;
	// }
	else {
		// queue_B_nodes[writeIndex].item = item;
		// writeIndex = (writeIndex + 1) % PART_B_MAX_SIZE;
...
/* Add code BEGIN */
	...
	// }
		...
		// *item = queue_B_nodes[readIndex].item;
		// readIndex = (readIndex + 1) % PART_B_MAX_SIZE;
		// bufferLength -= 1;
		// *item = queue_B_nodes[writeIndex - 1].item;
		*item = queue_B_nodes[0].item;
		bufferLength -= 1;
		while ((int curr = 0) < bufferLength) {
			queue_B_nodes[curr] = queue_B_nodes[curr + 1];
			curr++;
		}
		return 0;
	}
...
/* Add code BEGIN */
	...
	// 	return ERR_EMPTY;
	// }
	// else if (writeIndex - readIndex == PART_B_MAX_SIZE) {
	// 	return ERR_EMPTY;
	// }
	else {
		int result = -1;
		...
		while (currIndex >= 0) {
			...
			if (result == 0) {
				int old = currIndex;
				// int next = (old + 1) % PART_B_MAX_SIZE;
				int next = old + 1;
				while (next < bufferLength) {
				// while (tempBuffer > 0) {
					queue_B_nodes[old].item = queue_B_nodes[next].item;
					// old = (old + 1) % PART_B_MAX_SIZE;
					// next = (old + 1) % PART_B_MAX_SIZE;
					old = next;
					next += 1;
					// tempBuffer -= 1;
				}
			...
			// currIndex = (currIndex + 1) % PART_B_MAX_SIZE;
			currIndex--;
			// tempBuffer -= 1;
		}
		return ERR_NO_SUCH_ITEM;
